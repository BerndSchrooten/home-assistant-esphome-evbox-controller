...
SetCustomChargeCurrentAction = evbox_ns.class_("SetCustomChargeCurrentAction", automation.Action)
...
async def evbox_set_custom_charge_current_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, paren)
